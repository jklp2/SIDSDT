...
class DiscLoss():
    ...
    def Get_g_loss(self, net, real, fake):
        # First, G(A) should fake the discriminator
        pred_fake = net.forward(fake)
        return -self.criterionGAN(pred_fake, 0)
    ...
    def get_loss(self, net, realA=None, fakeB=None, realB=None):
        ...
        if realA is not None:
            loss_D_fake2 = self.criterionGAN(realA, 0)
        ################################################
        ...
        # Combined loss
        loss_D = (loss_D_fake2 + loss_D_fake + loss_D_real) * 0.5
        return loss_D, pred_fake, pred_real

    def Get_loss(self, net, real,fake):
        pred_fake = net.forward(fake)
        pred_real = net.forward(real)
        # Combined loss
        return self.criterionGAN(pred_real,1)+self.criterionGAN(pred_fake, 0)
...
def init_loss(opt, tensor):
    ...
    loss_dic['ssim'] = pytorch_ssim.SSIMloss()
    loss_dic['mse'] = nn.MSELoss()
    loss_dic['gan'] = GANLoss()
    loss_dic['l1'] = nn.L1Loss()
