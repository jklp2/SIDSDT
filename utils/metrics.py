...
if __name__ == '__main__':
    input_paths = glob.glob("/media/raid/vae_dehaze/res/res_dhaze/dcp/*")
    GT_paths = glob.glob("/media/raid/vae_dehaze/res/res_dhaze/dcp/*")
    input_paths = sorted(input_paths, key = lambda x: x.split('/')[-1].split('_')[0])
    GT_paths = sorted(GT_paths, key = lambda x: x.split('/')[-1].split('_')[0])

    print(input_paths)
    print(GT_paths)
    for i in range(len(GT_paths)) :
        input = cv2.imread(input_paths[i])
        GT = cv2.imread(GT_paths[i])
        print(quality_assess(input,GT))