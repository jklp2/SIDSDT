...
class SELayer_old(nn.Module):
    def __init__(self, channel, reduction=16):
        super(SELayer_old, self).__init__()
...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.combine = nn.Conv2d(channel,channel//2,3,padding=1)
...
class DRNet(torch.nn.Module):
    def __init__(self, in_channels, out_channels, n_feats, n_resblocks, norm=nn.BatchNorm2d,
                 se_reduction=None, res_scale=1, bottom_kernel_size=3, pyramid=False,isres=False,enc=True):
    ...
    def forward(self, input):
        ...
        if self.isres:
            if(self.enc):
                # pdb.set_trace()
                return torch.cat([input,x[...,:h,:w]],dim=1)
            else:
                return input[...,:3,:h,:w]+x[...,:h,:w]
        return x[...,:h,:w]
