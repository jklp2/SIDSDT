...
import itertools
from networks.unet_light import NestedUNet
...
class Model(Base):
    ...
    def initialize(self, opt):
        ...
        syn_encoder=NestedUNet(3, 16).cuda()
        syn_decoder=NestedUNet(16, 3).cuda()
        real_encoder=NestedUNet(3, 16).cuda()
        real_decoder=NestedUNet(16, 3).cuda()
    ...
    def forward_D(self):
        ...
        # self.real_dehaze=self.nets['syn_decoder'](self.fea_real)
        self.score_fake = self.nets['D'](self.real_dehaze.detach())
        self.score_real = self.nets['D'](self.target_syn)
    ...
    def get_Dloss(self):
        ...
        # self.loss_G+=self.loss_gan_G*0.0001
        # self.loss_G=self.loss_syn+self.loss_real+self.loss_cycle*2+self.loss_serd+self.loss_resd+self.loss_gan_G*0.0001
        self.loss_D += self.loss_gan_D * 0.0001
