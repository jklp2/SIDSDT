...
class Model(Base):
    ...
    def initialize(self, opt):
        ...
        if opt.skip:
            syn_decoder = DRNet(35, 3, 256, 1, nn.InstanceNorm2d, None, 1, 3, False, opt.skip,False).cuda()
        else:
        ...
        if opt.skip:
            real_decoder = DRNet(35, 3, 256, 1, nn.InstanceNorm2d, None, 1, 3, False, opt.skip,False).cuda()
        else:
            real_decoder=DRNet(32, 3, 256, 1, nn.InstanceNorm2d, None, 1, 3, False, opt.skip,False).cuda()
...
class Model_final_stage(Base):
    ...
    def initialize(self, opt):
        ...
        if opt.skip:
            ...
            refineNet = nn.Sequential(SELayer_old(70), GRUnet(70,105,5)).cuda()
        else:
            # refineNet = nn.Sequential(SELayer(64), DRNet(64, 32, 256, 3, nn.InstanceNorm2d, None, 1, 3, False, opt.skip)).cuda()
            refineNet = nn.Sequential(SELayer_old(64), GRUnet(64,32,5)).cuda()
