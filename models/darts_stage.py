...
import itertools
from networks.DRNet import SELayer
...
class Model(Base):
    ...
    def initialize(self, opt):
        ...
        syn_encoder=encoder(64,steps=3,oplist=[1, 1, 1, 7, 1, 7, 0, 0, 0, 0]).cuda()
        real_encoder=encoder(64,steps=3,oplist=[1, 1, 1, 7, 1, 7, 0, 0, 0, 0]).cuda()
        syn_decoder = decoder(64, steps=3, oplist=[7, 7, 7, 1, 0, 1, 7, 7, 7, 7]).cuda()
        real_decoder = decoder(64, steps=3, oplist=[7, 7, 7, 1, 0, 1, 7, 7, 7, 7]).cuda()
...
class Model_final_stage(Base):
    ...
    def initialize(self, opt):
        BaseModel.initialize(self,opt)
        syn_encoder = encoder(64, steps=2, oplist=[1, 1, 1, 7, 1, 7, 0, 0, 0, 0]).cuda()
        real_encoder = encoder(64, steps=2, oplist=[1, 1, 1, 7, 1, 7, 0, 0, 0, 0]).cuda()
        syn_decoder = decoder(64, steps=1, oplist=[7, 7, 7, 1, 0, 1, 7, 7, 7, 7]).cuda()
        if opt.skip:
            ...
            refineNet = nn.Sequential(SELayer(128), GRUnet(128,64,5)).cuda()
